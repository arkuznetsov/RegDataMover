...
//				<имя поля>			- Произвольный			- Значения измерений, ресурсов и реквизитов записи регистра
//
Функция ДвиженияДокументаВСтруктуру(Объект, СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Для Каждого ТекДвижения Из МетаОбъект.Движения Цикл
		Если НЕ ИспользоватьСвойство("Движения." + ТекДвижения.Имя, СписокСвойств, ИсключаяСвойства) Тогда
			Продолжить;
		КонецЕсли;
...
// Возвращаемое значение:
//
Процедура ДвиженияДокументаИзСтруктуры(Объект, СтруктураДвижений, СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
...
Процедура НаборЗаписейРегистраБухгалтерииИзСтруктуры(НаборЗаписей, СтруктураОбъекта, СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Для Каждого ТекЗапись Из СтруктураОбъекта.Записи Цикл
		...
		Для Каждого ТекПоле Из МетаОбъект.Ресурсы Цикл
			...
			КонецЕсли;
			Если МетаОбъект.Корреспонденция И НЕ ТекПоле.Балансовый Тогда
				НоваяЗапись[ТекПоле.Имя + "Дт"] = ЗначениеИзСтруктуры(ТекЗапись[ТекПоле.Имя + "Дт"]);
				НоваяЗапись[ТекПоле.Имя + "Кт"] = ЗначениеИзСтруктуры(ТекЗапись[ТекПоле.Имя + "Кт"]);
			Иначе
				НоваяЗапись[ТекПоле.Имя] = ЗначениеИзСтруктуры(ТекЗапись[ТекПоле.Имя]);
			КонецЕсли;
		КонецЦикла;
