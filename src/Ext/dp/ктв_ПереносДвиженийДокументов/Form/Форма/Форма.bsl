...
// Процедура - Обработки команды команды "ЗагрузитьДвижения"
//
&НаКлиенте
Процедура ЗагрузитьДвижения(Команда)
	...
	Пока Истина Цикл
		...
		ДопПараметры.ТекстДанных.НачатьЧтение(ОбработкаЧтенияФайла, ПутьКФайлу, КодировкаТекста.UTF8);
...
Функция ПолучитьИмяФайлаБезНомера(Знач ПутьКФайлу)
	...
	Возврат Сред(ИмяФайла, 1, СтрДлина(ИмяФайла) - СтрДлина(Формат(НомерФайла, "ЧГ=0")) + 1);
